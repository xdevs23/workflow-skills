...
This document defines the implementation requirements and review criteria.
...
- **Idempotent + reloadable.** A reload just re-subscribes → re-snapshot → same
  store. No special "initial load" code distinct from "live update" code. Correctness depends on replayable hub events, not transient browser state.
...
- **Normal-client model (own threads only):** browser does `whoami` +
  ...
  forces two code paths (initial load vs. live). The event stream must cover all hub content and use the same events for snapshots and updates.
...
- It can later gate **authorization**: `admin_subscribe` (the omniscient
  ...
  has no browser authentication, but the suffix is the natural seam to add it
...
The chosen aesthetic is **Aubergine** — defined by the
reference mockup `.cache/web-design-drafts/aubergine-dark.html`. That mockup is a
...
register/release alias, browse directory & history. Destructive ops (leave, release_alias) are in the UI but behind the
...
- **Browser authentication** — outside v1 scope. The
