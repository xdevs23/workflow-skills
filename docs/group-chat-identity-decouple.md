...
| Decouple scope | **Full decouple now** | A rename alone leaves identity coupled to session lifetime. Decouple first, then build reply-to. |
...
- **An identity MAY have several live sessions** — `identityConns[identity]` is a `Set`
  ...
  enforce "one active session per identity" (single-session eviction would break concurrent connections).
...
- **Keep session id as identity, just rename the column** (`owner_session` →
  `owner_identity` with the same value). Rejected: the conflation stays in storage and a `/resume` still fragments identity.
...
- **Reply carries a denormalized quote snapshot.** Rejected: the sequence reference is sufficient; the recipient/display can look up the original from history.
...
- **One active session per identity (evict the prior on any new bind).** Rejected: subagents share the parent session id, so there's no concurrent
...
- **Make subagents the same identity as their parent (no `agent_id`).** Rejected: the composite session key distinguishes each subagent as its own participant. Session id alone can't tell them apart.
